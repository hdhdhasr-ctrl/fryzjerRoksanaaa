...
export default function App() {
  ...
  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({ ...prev, time: sanitizedVal }));
    validateTime(sanitizedVal, formData.date);
  };
    ...
    if (formData.date) {
      ...
      if (formData.date < todayStr) {
        ...
        if (schedule.isClosed) {
          ...
          validateTime(formData.time, formData.date);
        }
      }
    }
  }, [formData.date]);
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-1">
                            ...
                            <input 
                              ...
                              className={`w-full bg-black border ${formErrors.name ? 'border-red-500' : 'border-white/10'} p-4 text-sm text-white focus:border-[#C5A059] outline-none transition-colors rounded-none placeholder:text-gray-600`}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-6 border-t border-white/5">
                          ...
                          <div className="md:col-span-5 bg-black border border-[#C5A059]/30 p-5 space-y-3 flex flex-col justify-between">
                            ...
                            <div className="relative">
                              <input 
                                type="time" 
                                min={getMinTimeForDate(formData.date) || undefined}
                                max={getDaySchedule().maxTime || undefined}
                                disabled={getDaySchedule().isClosed}
                                required={!getDaySchedule().isClosed}
                                value={formData.time}
                                onChange={handleTimeChange}
                                className={`w-full bg-black border ${formErrors.time || timeError ? 'border-red-500' : 'border-[#C5A059]/40'} p-3.5 text-sm text-white focus:border-[#C5A059] outline-none transition-colors rounded-none disabled:opacity-50 disabled:cursor-not-allowed`}
                              />
                            </div>
